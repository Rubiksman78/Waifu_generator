...
def normalize(input_image,true_mask):
    ...
    true_mask = (tf.cast(true_mask,tf.float32)-127.5)/127.5
    return input_image,true_mask

def load_image(datapoint,im_size=IMG_HEIGHT):
    ...
    return input_image,true_mask,input_mask
...
class Augment(tf.keras.layers.Layer):
    def __init__(self, seed=42):
        ...
        self.augment_inputs = tf.keras.Sequential([layers.RandomFlip(mode="horizontal", seed=seed),layers.RandomRotation(0.1,seed=seed)])
        self.augment_labels = tf.keras.Sequential([layers.RandomFlip(mode="horizontal", seed=seed),layers.RandomRotation(0.1,seed=seed)])
        self.augment_true_labels = tf.keras.Sequential([layers.RandomFlip(mode="horizontal", seed=seed),layers.RandomRotation(0.1,seed=seed)])    
    
    def call(self, inputs, labels ,true_labels):
        inputs = self.augment_inputs(inputs)
        labels = self.augment_labels(labels)
        true_labels = self.augment_true_labels(true_labels)
        return inputs, true_labels, labels
...
test_batches = test_images.batch(batch_size, drop_remainder=True).map(One_Hot())
